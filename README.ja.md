...
- **InkOS Studio**（v1.0）：`inkos studio` でローカルWebワークベンチを起動（Vite + React + Hono）。書籍管理、章レビュー・編集、リアルタイム執筆進捗、マーケットレーダー、データ分析、AI検出、文体分析、ジャンル管理、デーモン制御、真実ファイル編集——CLIの全機能をビジュアル化
- **Foundation Reviewer**（v1.1）：書籍作成時に独立した審査Agent、5次元100点制スコアリング、80点未満は自動却下して再生成
- **Hook Seed Excerpt**（v1.1）：伏線回収時、Composerが原始シーンの原文抜粋をWriter コンテキストに注入、具体的な素材に基づいて回収シーンを執筆
- **Review Reject ロールバック**（v1.1）：`inkos review reject` が却下章の前のスナップショットまでstateをロールバック、下流の章とメモリインデックスを破棄
- **State Validation Recovery**（v1.1）：stateバリデーション失敗時にsettlerを自動リトライ、それでも失敗時は正常降格で保存、`inkos write repair-state` で手動修復
- **バイリンガルインポート**（v1.1.1）：`import chapters` と `fanfic init` がバイリンガルプロンプトに対応、続編/シリーズモードを自動検出
- **章番号アンカリング**（v1.1）：章の進捗は連続した永続ファイルのみを権威ソースとし、叙述テキスト内の数字が進捗を汚染しない
- 監査ドリフト隔離、タイトル崩壊修復、Hook予算ヒント、章末サマリー、ムード/ペース単調検出
- AI-tells・センシティブワードのバイリンガル対応、カスタムHTTPヘッダー（`INKOS_LLM_HEADERS`）
