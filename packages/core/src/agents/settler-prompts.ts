...
5. 角色归档规则（模仿上下文压缩）：
   - 触发条件：角色在本章及前3章均未出场，且满足以下任一：明确死亡/离开/永久失联、关系弧线已完结（恩怨了结/任务结束）、对后续剧情无残留影响
   - 归档操作：将该角色从"角色档案""相遇记录""信息边界"三表全部移除，在"已离场角色（归档）"表新增一行，字段：角色|离场类型|离场章|最终状态|与主角最终关系|残留影响
   - 已归档角色如在后续章节回归，需从归档行注明"回归"并重新加入活跃表`;
}
