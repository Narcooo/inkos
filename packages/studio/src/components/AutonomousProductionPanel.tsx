...
export interface AutonomousView {
  ...
  readonly runtime?: { readonly lastError?: string; readonly phase?: string; readonly activeRole?: string; readonly activeProvider?: string | null; readonly activeModel?: string | null; readonly updatedAt?: string; readonly repairOutcome?: { readonly chapter: number; readonly status: string; readonly errorCode: string | null; readonly reservedCostUpperUsd?: number } } | null;
...
export function AutonomousProductionCard({ view, pending, error, onStart, onStop, onRepair, onConfigureModels }: {
  ...
  return <section data-testid="autonomous-production" className="paper-sheet rounded-2xl border border-primary/20 p-5 shadow-sm space-y-4">
    ...
    <div className="grid grid-cols-2 gap-3 text-xs md:grid-cols-3"><div><span className="text-muted-foreground">Remaining Volume Forecast</span><div className="font-semibold">{forecastRange(view.economics.remainingVolumeForecast ?? view.economics.currentVolumeForecast)}</div></div><div><span className="text-muted-foreground">Current Volume Estimated Total</span><div className="font-semibold">{forecastRange(view.economics.currentVolumeEstimatedTotal ?? view.economics.currentVolumeForecast)}</div></div><div><span className="text-muted-foreground">Current Volume Hard Cap</span><div className="font-semibold">{money(view.budget.hardCapUsd)}</div></div></div>
    {repairNeedsReconciliation ? <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-sm">A persisted state-repair attempt requires reconciliation. No new repair transport will start.</div>
      ...
      : repair ? <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-sm"><p>Chapter {ch(Number(repair[1]))} state requires repair before production can continue.</p><p className="mt-1 text-xs text-muted-foreground">Bounded repair forecast: {view.economics.repairForecast ? forecastRange(view.economics.repairForecast) : "Unavailable"}. Uses the existing settlement and Observer / Reflector validation path.</p><button disabled={pending || active || view.economics.repairForecast?.highUsd == null} onClick={() => onRepair(Number(repair[1]))} className="mt-3 rounded-lg bg-amber-600 px-4 py-2 font-bold text-white disabled:opacity-40">Repair Chapter {ch(Number(repair[1]))} State</button></div>
      : view.chapterAttention?.status === "AUDIT_FAILED_STATE_SETTLED" ? <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-sm">Chapter {ch(view.chapterAttention.chapter)} state is settled; independent review still failed.</div>
