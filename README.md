...
**模型配置** — Studio 内置多服务配置、模型路由和封面服务配置；也支持 [kkaiapi](https://kkaiapi.com/) / OpenRouter / [Atlas Cloud](https://www.atlascloud.ai/) 等全球主流模型聚合入口，以及自定义 OpenAI-compatible 服务。
...
- **Provider bank 能力表**：内置 Google Gemini、Moonshot、MiniMax、智谱、百炼、DeepSeek、硅基流动、火山、腾讯混元、文心、讯飞星火、OpenRouter、kkaiapi、Atlas Cloud、Ollama、CodingPlan 等服务的 baseUrl、协议、模型和兼容策略。
